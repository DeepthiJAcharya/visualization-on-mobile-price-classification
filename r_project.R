...
legend("bottomright",c("0(Low Cost)","1(Medium Cost)","2(High 
Cost)","3(Very High Cost)"),cex=0.8,fill=colors,bg="grey") 
dev.off()

#3D pie chart
png("3Dpie_chart.png")
piepercent = round(100*freq/sum(freq))
pie3D(freq,labels=piepercent,col=colors, main="3D Pie Chart 
Representing Price Range",explode=0.05)
legend("topright", c("0(Low Cost)","1(Medium Cost)","2(High 
Cost)","3(Very High Cost)"), cex=0.8, fill=colors, bg="grey")
dev.off()
